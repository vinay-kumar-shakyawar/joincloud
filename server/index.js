...
function getActivationPaths() {
  const userData = process.env.JOINCLOUD_USER_DATA || os.homedir();
...
function getUsagePath() {
  const paths = getActivationPaths();
  if (!paths) return null;
  return path.join(paths.systemDir, "usage.json");
}
...
function writeLocalUsage(payload) {
  const usagePath = getUsagePath();
  const paths = getActivationPaths();
...
function getProfilePath() {
  const userData = process.env.JOINCLOUD_USER_DATA;
  if (!userData) return null;
  return path.join(userData, "JoinCloud", "system", "profile.json");
}

function ensureProfileAndGetDisplayName() {
  const profilePath = getProfilePath();
  if (!profilePath) return null;
  try {
    if (fsSync.existsSync(profilePath)) {
      const raw = fsSync.readFileSync(profilePath, "utf8");
      const parsed = JSON.parse(raw);
      return typeof parsed.displayName === "string" && parsed.displayName ? parsed.displayName : null;
    }
    const hostname = os.hostname();
    const cleaned = (hostname || "").replace(/[^a-zA-Z0-9]/g, "") || "Device";
    const displayName = `Join ${cleaned}`;
    const dir = path.dirname(profilePath);
    if (!fsSync.existsSync(dir)) fsSync.mkdirSync(dir, { recursive: true });
    fsSync.writeFileSync(profilePath, JSON.stringify({ displayName, customSuffix: cleaned }, null, 2), "utf8");
    return displayName;
  } catch (_) {
    return null;
  }
...
function generateDisplayName() {
  const fromProfile = ensureProfileAndGetDisplayName();
  if (fromProfile) return normalizeDisplayName(fromProfile);
  try {
    const hostname = os.hostname();
    const cleaned = (hostname || "").replace(/[^a-zA-Z0-9]/g, "") || "Device";
    return `Join ${cleaned}`;
...
async function bootstrap() {
  ...
  app.post("/api/v1/devices/bootstrap-trial", (req, res) => {
    const adminHost = config.telemetry.adminHost;
    if (!adminHost) {
      res.status(503).json({ message: "Control Plane not configured. Set JOINCLOUD_ADMIN_HOST." });
      return;
    ...
    controlPlanePost(adminHost, "/api/v1/devices/bootstrap-trial", { deviceId }, null, (err, result) => {
      if (err) {
        logger.warn("bootstrap-trial proxy error", { error: err.message });
        res.status(502).json({ message: "Could not reach Control Plane" });
        return;
      }
  ...
  app.post("/api/v1/devices/usage/shares/increment", (req, res) => {
    ...
    controlPlanePost(adminHost, "/api/v1/devices/usage/shares/increment", { deviceId }, null, (err, result) => {
      if (err) {
        logger.warn("usage/shares/increment proxy error", { error: err.message });
        res.status(502).json({ message: "Could not reach Control Plane" });
        return;
      }
...
<html lang="en">
  ...
  app.post("/api/share", async (req, res) => {
    try {
      ...
      // Strict rule: block all sharing when internet is unavailable.
      const online = await isInternetOnline();
      if (!online) {
        res.status(403).json({
          error: "offline",
          message: "Sharing is disabled while internet is unavailable. Please reconnect and try again.",
        });
        return;
      }
      const licenseState = getLicenseStateFromLocalState();
  ...
  app.post("/api/v1/network/settings", async (req, res) => {
    ...
    if (updated) {
      await updateUserConfig(config.storage.userConfigPath, userConfig);
      if (typeof requestedName === "string" && isValidDisplayName(requestedName)) {
        const profilePath = getProfilePath();
        if (profilePath) {
          try {
            const suffix = requestedName.trim().toLowerCase().startsWith("join")
              ? requestedName.trim().slice(4).trim()
              : requestedName.trim();
            const customSuffix = suffix || "Device";
            const displayName = suffix ? `Join ${suffix}` : "Join";
            const dir = path.dirname(profilePath);
            if (!fsSync.existsSync(dir)) fsSync.mkdirSync(dir, { recursive: true });
            let existing = {};
            if (fsSync.existsSync(profilePath)) {
              try {
                existing = JSON.parse(fsSync.readFileSync(profilePath, "utf8"));
              } catch (_) {}
            }
            fsSync.writeFileSync(profilePath, JSON.stringify({ ...existing, displayName, customSuffix }, null, 2), "utf8");
          } catch (_) {}
        }
      }
    }
  ...
  app.get("/api/v1/control-plane-config", (req, res) => {
    const requestedDeviceId = typeof req.query?.deviceId === "string" ? String(req.query.deviceId).trim() : "";
    const hostUuidForCfg = (requestedDeviceId && requestedDeviceId.length >= 8)
      ? requestedDeviceId
      : (getHostUuidForConfig ? getHostUuidForConfig() : null);
    const adminHost = config.telemetry?.adminHost;
    ...
    function sendResponse(cp) {
      ...
      if (cached && cached.display_name) out.display_name = out.display_name || cached.display_name;
      const paths = getActivationPaths();
      let isAuthenticated = false;
      if (paths && fsSync.existsSync(paths.authPath)) {
        try {
          const raw = fsSync.readFileSync(paths.authPath, "utf8");
          const auth = JSON.parse(raw);
          isAuthenticated = !!(auth && typeof auth.token === "string" && auth.token.trim().length > 0);
        } catch (_) {}
      }
      out.is_authenticated = isAuthenticated;
      tryRefreshLicense();
    ...
    let cp = getControlPlaneConfig();
    if (!cp || !cp.license || !cp.license.state) {
      cp = applyConfigFromControlPlane();
    }
    sendResponse(cp || { license: {}, activation: {}, telemetry: {} });
  });
    ...
    controlPlanePost(adminHost, "/api/desktop/verify", { token }, null, (err, result) => {
      ...
      try {
        ...
        if (paths) {
          if (!fsSync.existsSync(paths.systemDir)) fsSync.mkdirSync(paths.systemDir, { recursive: true });
          if (data.jwt) {
            fsSync.writeFileSync(paths.authPath, JSON.stringify({ token: data.jwt }, null, 2), "utf8");
          }
        }
        if (data.license && typeof data.license === "object") {
          ...
          if (paths) fsSync.writeFileSync(paths.licensePath, JSON.stringify(licenseToSave, null, 2), "utf8");
          controlPlaneConfigCache = {
  ...
  // Browser loopback callback for desktop auth/extend flows.
  // This avoids fragile custom protocol handlers in dev/packaged environments.
  app.get("/auth/callback", (req, res) => {
    ...
    if (!fromLocalhost && !hostIsLocal) {
      res.status(403).send(`<!doctype html><html><head><meta charset="utf-8"/><title>JoinCloud</title><style>:root{--bg:#000405;--section-bg:#00080A;--stroke:#001C25;--primary:#2FB7FF;--foreground:#FFFFFF;--foreground-soft:#8B9CA3;--error:#EF4444}*{box-sizing:border-box;margin:0;padding:0}body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:var(--bg);color:var(--foreground);min-height:100vh}.header{display:flex;align-items:center;gap:10px;padding:12px 16px;background:#000A0D;border-bottom:1px solid var(--stroke)}.logo{width:26px;height:26px;background:var(--primary);border-radius:6px;display:flex;align-items:center;justify-content:center}.logo svg{width:18px;height:18px}.brand{font-weight:700;font-size:18px}.content{display:flex;align-items:center;justify-content:center;padding:48px 24px;min-height:calc(100vh - 51px)}.card{background:var(--section-bg);border:1px solid var(--stroke);border-radius:12px;padding:32px;max-width:400px;width:100%;text-align:center}.icon{width:48px;height:48px;background:rgba(239,68,68,0.15);border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 16px}h2{font-size:22px;font-weight:700;margin-bottom:8px}p{color:var(--foreground-soft);font-size:14px;line-height:1.5}</style></head><body><div class="header"><div class="logo"><svg viewBox="0 0 180 180" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill="#000405" d="M101.141 53H136.632C151.023 53 162.689 64.6662 162.689 79.0573V112.904H148.112V79.0573C148.112 78.7105 148.098 78.3662 148.072 78.0251L112.581 112.898C112.701 112.902 112.821 112.904 112.941 112.904H148.112V126.672H112.941C98.5504 126.672 86.5638 114.891 86.5638 100.5V66.7434H101.141V100.5C101.141 101.15 101.191 101.792 101.289 102.422L137.56 66.7816C137.255 66.7563 136.945 66.7434 136.632 66.7434H101.141V53Z"/><path fill="#000405" d="M65.2926 124.136L14 66.7372H34.6355L64.7495 100.436V66.7372H80.1365V118.47C80.1365 126.278 70.4953 129.958 65.2926 124.136Z"/></svg></div><span class="brand">JoinCloud</span></div><div class="content"><div class="card"><div class="icon"><svg width="24" height="24" fill="none" stroke="#EF4444" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg></div><h2>Forbidden</h2><p>This callback is only available on local desktop.</p></div></div></body></html>`);
      return;
    }
    const token = typeof req.query?.token === "string" ? req.query.token : "";
    const mode = typeof req.query?.mode === "string" ? req.query.mode : "login";
    if (!token) {
      res.status(400).send(`<!doctype html><html><head><meta charset="utf-8"/><title>JoinCloud</title><style>:root{--bg:#000405;--section-bg:#00080A;--stroke:#001C25;--primary:#2FB7FF;--foreground:#FFFFFF;--foreground-soft:#8B9CA3;--error:#EF4444}*{box-sizing:border-box;margin:0;padding:0}body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:var(--bg);color:var(--foreground);min-height:100vh}.header{display:flex;align-items:center;gap:10px;padding:12px 16px;background:#000A0D;border-bottom:1px solid var(--stroke)}.logo{width:26px;height:26px;background:var(--primary);border-radius:6px;display:flex;align-items:center;justify-content:center}.logo svg{width:18px;height:18px}.brand{font-weight:700;font-size:18px}.content{display:flex;align-items:center;justify-content:center;padding:48px 24px;min-height:calc(100vh - 51px)}.card{background:var(--section-bg);border:1px solid var(--stroke);border-radius:12px;padding:32px;max-width:400px;width:100%;text-align:center}.icon{width:48px;height:48px;background:rgba(239,68,68,0.15);border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 16px}h2{font-size:22px;font-weight:700;margin-bottom:8px}p{color:var(--foreground-soft);font-size:14px;line-height:1.5}</style></head><body><div class="header"><div class="logo"><svg viewBox="0 0 180 180" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill="#000405" d="M101.141 53H136.632C151.023 53 162.689 64.6662 162.689 79.0573V112.904H148.112V79.0573C148.112 78.7105 148.098 78.3662 148.072 78.0251L112.581 112.898C112.701 112.902 112.821 112.904 112.941 112.904H148.112V126.672H112.941C98.5504 126.672 86.5638 114.891 86.5638 100.5V66.7434H101.141V100.5C101.141 101.15 101.191 101.792 101.289 102.422L137.56 66.7816C137.255 66.7563 136.945 66.7434 136.632 66.7434H101.141V53Z"/><path fill="#000405" d="M65.2926 124.136L14 66.7372H34.6355L64.7495 100.436V66.7372H80.1365V118.47C80.1365 126.278 70.4953 129.958 65.2926 124.136Z"/></svg></div><span class="brand">JoinCloud</span></div><div class="content"><div class="card"><div class="icon"><svg width="24" height="24" fill="none" stroke="#EF4444" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg></div><h2>Invalid callback</h2><p>Missing token.</p></div></div></body></html>`);
      return;
    }
      ...
      (resp) => {
        ...
        resp.on("end", () => {
          ...
          if (resp.statusCode !== 200) {
            ...
            res.status(resp.statusCode || 401).send(`<!doctype html><html><head><meta charset="utf-8"/><title>JoinCloud</title><style>:root{--bg:#000405;--section-bg:#00080A;--stroke:#001C25;--primary:#2FB7FF;--foreground:#FFFFFF;--foreground-soft:#8B9CA3;--error:#EF4444}*{box-sizing:border-box;margin:0;padding:0}body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:var(--bg);color:var(--foreground);min-height:100vh}.header{display:flex;align-items:center;gap:10px;padding:12px 16px;background:#000A0D;border-bottom:1px solid var(--stroke)}.logo{width:26px;height:26px;background:var(--primary);border-radius:6px;display:flex;align-items:center;justify-content:center}.logo svg{width:18px;height:18px}.brand{font-weight:700;font-size:18px}.content{display:flex;align-items:center;justify-content:center;padding:48px 24px;min-height:calc(100vh - 51px)}.card{background:var(--section-bg);border:1px solid var(--stroke);border-radius:12px;padding:32px;max-width:400px;width:100%;text-align:center}.icon{width:48px;height:48px;background:rgba(239,68,68,0.15);border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 16px}h2{font-size:22px;font-weight:700;margin-bottom:8px}p{color:var(--foreground-soft);font-size:14px;line-height:1.5;margin-bottom:8px}p:last-child{margin-bottom:0}</style></head><body><div class="header"><div class="logo"><svg viewBox="0 0 180 180" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill="#000405" d="M101.141 53H136.632C151.023 53 162.689 64.6662 162.689 79.0573V112.904H148.112V79.0573C148.112 78.7105 148.098 78.3662 148.072 78.0251L112.581 112.898C112.701 112.902 112.821 112.904 112.941 112.904H148.112V126.672H112.941C98.5504 126.672 86.5638 114.891 86.5638 100.5V66.7434H101.141V100.5C101.141 101.15 101.191 101.792 101.289 102.422L137.56 66.7816C137.255 66.7563 136.945 66.7434 136.632 66.7434H101.141V53Z"/><path fill="#000405" d="M65.2926 124.136L14 66.7372H34.6355L64.7495 100.436V66.7372H80.1365V118.47C80.1365 126.278 70.4953 129.958 65.2926 124.136Z"/></svg></div><span class="brand">JoinCloud</span></div><div class="content"><div class="card"><div class="icon"><svg width="24" height="24" fill="none" stroke="#EF4444" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg></div><h2>Could not finish sign-in</h2><p>${msg}</p><p>Please return to JoinCloud and try again.</p></div></div></body></html>`);
            return;
          }
          // Electron main process watches this marker in backend stdout and emits license-updated IPC.
          console.log("[joincloud-auth-callback] license-updated");
          const title = mode === "extendTrial" ? "Trial extended" : "Sign-in successful";
          res.status(200).send(`<!doctype html><html><head><meta charset="utf-8"/><title>JoinCloud</title><style>:root{--bg:#000405;--section-bg:#00080A;--stroke:#001C25;--primary:#2FB7FF;--foreground:#FFFFFF;--foreground-soft:#8B9CA3}*{box-sizing:border-box;margin:0;padding:0}body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:var(--bg);color:var(--foreground);min-height:100vh}.header{display:flex;align-items:center;gap:10px;padding:12px 16px;background:#000A0D;border-bottom:1px solid var(--stroke)}.logo{width:26px;height:26px;background:var(--primary);border-radius:6px;display:flex;align-items:center;justify-content:center}.logo svg{width:18px;height:18px}.brand{font-weight:700;font-size:18px}.content{display:flex;align-items:center;justify-content:center;padding:48px 24px;min-height:calc(100vh - 51px)}.card{background:var(--section-bg);border:1px solid var(--stroke);border-radius:12px;padding:32px;max-width:400px;width:100%;text-align:center}.icon{width:48px;height:48px;background:rgba(34,197,94,0.15);border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 16px}h2{font-size:22px;font-weight:700;margin-bottom:8px}p{color:var(--foreground-soft);font-size:14px;line-height:1.5}</style></head><body><div class="header"><div class="logo"><svg viewBox="0 0 180 180" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill="#000405" d="M101.141 53H136.632C151.023 53 162.689 64.6662 162.689 79.0573V112.904H148.112V79.0573C148.112 78.7105 148.098 78.3662 148.072 78.0251L112.581 112.898C112.701 112.902 112.821 112.904 112.941 112.904H148.112V126.672H112.941C98.5504 126.672 86.5638 114.891 86.5638 100.5V66.7434H101.141V100.5C101.141 101.15 101.191 101.792 101.289 102.422L137.56 66.7816C137.255 66.7563 136.945 66.7434 136.632 66.7434H101.141V53Z"/><path fill="#000405" d="M65.2926 124.136L14 66.7372H34.6355L64.7495 100.436V66.7372H80.1365V118.47C80.1365 126.278 70.4953 129.958 65.2926 124.136Z"/></svg></div><span class="brand">JoinCloud</span></div><div class="content"><div class="card"><div class="icon"><svg width="24" height="24" fill="none" stroke="#22C55E" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg></div><h2>${title}</h2><p>Success! You can close this tab and return to JoinCloud.</p></div></div></body></html>`);
        });
      }
    );
    req2.on("error", () => {
      res.status(502).send(`<!doctype html><html><head><meta charset="utf-8"/><title>JoinCloud</title><style>:root{--bg:#000405;--section-bg:#00080A;--stroke:#001C25;--primary:#2FB7FF;--foreground:#FFFFFF;--foreground-soft:#8B9CA3;--warning:#F59E0B}*{box-sizing:border-box;margin:0;padding:0}body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:var(--bg);color:var(--foreground);min-height:100vh}.header{display:flex;align-items:center;gap:10px;padding:12px 16px;background:#000A0D;border-bottom:1px solid var(--stroke)}.logo{width:26px;height:26px;background:var(--primary);border-radius:6px;display:flex;align-items:center;justify-content:center}.logo svg{width:18px;height:18px}.brand{font-weight:700;font-size:18px}.content{display:flex;align-items:center;justify-content:center;padding:48px 24px;min-height:calc(100vh - 51px)}.card{background:var(--section-bg);border:1px solid var(--stroke);border-radius:12px;padding:32px;max-width:400px;width:100%;text-align:center}.icon{width:48px;height:48px;background:rgba(245,158,11,0.15);border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 16px}h2{font-size:22px;font-weight:700;margin-bottom:8px}p{color:var(--foreground-soft);font-size:14px;line-height:1.5}</style></head><body><div class="header"><div class="logo"><svg viewBox="0 0 180 180" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill="#000405" d="M101.141 53H136.632C151.023 53 162.689 64.6662 162.689 79.0573V112.904H148.112V79.0573C148.112 78.7105 148.098 78.3662 148.072 78.0251L112.581 112.898C112.701 112.902 112.821 112.904 112.941 112.904H148.112V126.672H112.941C98.5504 126.672 86.5638 114.891 86.5638 100.5V66.7434H101.141V100.5C101.141 101.15 101.191 101.792 101.289 102.422L137.56 66.7816C137.255 66.7563 136.945 66.7434 136.632 66.7434H101.141V53Z"/><path fill="#000405" d="M65.2926 124.136L14 66.7372H34.6355L64.7495 100.436V66.7372H80.1365V118.47C80.1365 126.278 70.4953 129.958 65.2926 124.136Z"/></svg></div><span class="brand">JoinCloud</span></div><div class="content"><div class="card"><div class="icon"><svg width="24" height="24" fill="none" stroke="#F59E0B" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg></div><h2>Desktop app unavailable</h2><p>Please open JoinCloud first, then try again.</p></div></div></body></html>`);
    });
    req2.on("timeout", () => {
      req2.destroy();
      res.status(504).send(`<!doctype html><html><head><meta charset="utf-8"/><title>JoinCloud</title><style>:root{--bg:#000405;--section-bg:#00080A;--stroke:#001C25;--primary:#2FB7FF;--foreground:#FFFFFF;--foreground-soft:#8B9CA3;--warning:#F59E0B}*{box-sizing:border-box;margin:0;padding:0}body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:var(--bg);color:var(--foreground);min-height:100vh}.header{display:flex;align-items:center;gap:10px;padding:12px 16px;background:#000A0D;border-bottom:1px solid var(--stroke)}.logo{width:26px;height:26px;background:var(--primary);border-radius:6px;display:flex;align-items:center;justify-content:center}.logo svg{width:18px;height:18px}.brand{font-weight:700;font-size:18px}.content{display:flex;align-items:center;justify-content:center;padding:48px 24px;min-height:calc(100vh - 51px)}.card{background:var(--section-bg);border:1px solid var(--stroke);border-radius:12px;padding:32px;max-width:400px;width:100%;text-align:center}.icon{width:48px;height:48px;background:rgba(245,158,11,0.15);border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 16px}h2{font-size:22px;font-weight:700;margin-bottom:8px}p{color:var(--foreground-soft);font-size:14px;line-height:1.5}</style></head><body><div class="header"><div class="logo"><svg viewBox="0 0 180 180" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill="#000405" d="M101.141 53H136.632C151.023 53 162.689 64.6662 162.689 79.0573V112.904H148.112V79.0573C148.112 78.7105 148.098 78.3662 148.072 78.0251L112.581 112.898C112.701 112.902 112.821 112.904 112.941 112.904H148.112V126.672H112.941C98.5504 126.672 86.5638 114.891 86.5638 100.5V66.7434H101.141V100.5C101.141 101.15 101.191 101.792 101.289 102.422L137.56 66.7816C137.255 66.7563 136.945 66.7434 136.632 66.7434H101.141V53Z"/><path fill="#000405" d="M65.2926 124.136L14 66.7372H34.6355L64.7495 100.436V66.7372H80.1365V118.47C80.1365 126.278 70.4953 129.958 65.2926 124.136Z"/></svg></div><span class="brand">JoinCloud</span></div><div class="content"><div class="card"><div class="icon"><svg width="24" height="24" fill="none" stroke="#F59E0B" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg></div><h2>Timed out</h2><p>Please return to JoinCloud and retry.</p></div></div></body></html>`);
    });
    req2.write(body);
    req2.end();
  });
  ...
  app.post("/api/v1/auth/logout", (req, res) => {
    ...
    if (paths) {
      try {
        ...
        logger && logger.warn("logout auth cleanup failed", { error: e.message });
      }
    }
    controlPlaneConfigCache = null;
    res.json({ success: true });
